...
for split in SPLITS:
    ...
    # seg_tgt = []
    new_dataset = []
    with open(f'./data/qmsum_speaker_{split}.jsonl') as f:
        for line in tqdm(f):
            utter = []
            utter_speaker = []
            ...
            for segments in cluster_data:
                ...
                for s in segments:
                    query = "summarize: " + s
                    combined_summ.append(summarizer(query, min_length=MIN_TARGET_LEN, max_length=max_tgt_len))
